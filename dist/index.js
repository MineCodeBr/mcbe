...
// index.ts
var mcbe_exports = {};
__export(mcbe_exports, {
  Addon: () => Addon_default,
  Block: () => Block_default,
  Item: () => Item_default,
  Recipe: () => Recipe_default,
  default: () => mcbe_default
});
module.exports = __toCommonJS(mcbe_exports);
...
var Block_default = class {
  ...
  setTerrainTexture(name, value, src) {
    this.run["executeTerrainTexture"] = (v) => {
      if (!import_node_fs.default.existsSync(`${v.dirRe} RE/textures/blocks`))
        import_node_fs.default.mkdirSync(`${v.dirRe} RE/textures/blocks`, { recursive: true });
      let data = {
        num_mip_levels: 4,
        padding: 8,
      ...
      if (import_node_fs.default.existsSync(`${v.dirRe} RE/textures/terrain_texture.json`))
        data = JSON.parse(
          String(import_node_fs.default.readFileSync(`${v.dirRe} RE/textures/terrain_texture.json`))
        );
      import_node_fs.default.copyFileSync(src, `${v.dirRe} RE/${value}.png`);
  ...
  setGeometry(file, boneVisibility) {
    if (!import_node_fs.default.existsSync(file)) log_error("Geometria file nao existe");
    const readGeo = JSON.parse(String(import_node_fs.default.readFileSync(file)));
    if (boneVisibility) {
    ...
    this.run["executeGemotry"] = (v) => {
      if (!import_node_fs.default.existsSync(`${v.dirRe} RE/models/blocks`))
        import_node_fs.default.mkdirSync(`${v.dirRe} RE/models/blocks`, { recursive: true });
      import_node_fs.default.writeFileSync(`${v.dirRe} RE/models/blocks/${file.split("/").pop()}`, stringify(JSON.parse(String(import_node_fs.default.readFileSync(`${file}`))), v.build));
    };
    return this;
  }
  setCustomComponent(customName, customComponent, options) {
    ...
    this.run["set_custom_component"] = (v) => {
      ...
      if (v.build) {
        import_node_fs.default.appendFileSync(`${v.dirBe} BE/scripts/index.js`, js_stringify(data.replaceAll(`import * as mc from "@minecraft/server";`, ""), v.build));
        return this;
      }
      if (!import_node_fs.default.existsSync(`${v.dirBe} BE/scripts/blocks`)) import_node_fs.default.mkdirSync(`${v.dirBe} BE/scripts/blocks`, { recursive: true });
      import_node_fs.default.writeFileSync(`${v.dirBe} BE/scripts/blocks/${this.filename}.js`, js_stringify(data, v.build));
    };
    return this;
  }
};
...
var Item_default = class {
  ...
  setItemTexture(name, value, src) {
    this.run["set_item_texture"] = (v) => {
      if (!import_node_fs2.default.existsSync(`${v.dirRe} RE/textures/items`))
        import_node_fs2.default.mkdirSync(`${v.dirRe} RE/textures/items`, { recursive: true });
      let data = {
        "resource_pack_name": "wiki",
        "texture_name": "atlas.items",
        texture_data: {}
      };
      if (import_node_fs2.default.existsSync(`${v.dirRe} RE/textures/item_texture.json`))
        data = JSON.parse(
          String(import_node_fs2.default.readFileSync(`${v.dirRe} RE/textures/item_texture.json`))
        );
      import_node_fs2.default.copyFile(src, `${v.dirRe} RE/${value}.png`, (err) => {
        if (err) log_error(`Textura definida nao existe ${import_cli_color3.default.greenBright(src)}`);
      });
      data["texture_data"][name] = { textures: value };
  ...
  setCustomComponent(customName, customComponent) {
    ...
    this.run["set_custom_component"] = (v) => {
      ...
      if (v.build) {
        import_node_fs2.default.appendFileSync(`${v.dirBe} BE/scripts/index.js`, js_stringify(data.replaceAll(`import * as mc from "@minecraft/server";`, ""), v.build));
        return this;
      }
      if (!import_node_fs2.default.existsSync(`${v.dirBe} BE/scripts/items`))
        import_node_fs2.default.mkdirSync(`${v.dirBe} BE/scripts/items`, { recursive: true });
      import_node_fs2.default.writeFileSync(`${v.dirBe} BE/scripts/items/${this.filename}.js`, js_stringify(data, v.build));
    };
    return this;
  }
};
